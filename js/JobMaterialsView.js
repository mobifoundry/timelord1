...
//--------------------------------------------------------------------------------------// 
function JobMaterialsView(){
 if ( MaterialsView == null )  {
    MaterialsView = new ListView(MENU_ITEM_MATERIALS, MaterialsButtonText);
    materialsField = new TextField(null,LBL008);	
    MaterialsView.addControl(materialsField);
    MaterialsPanel = new ContentPanel(null,LBL007 + totalmaterials[Jobnum],RegisterTape[Jobnum], true, false);		
    MaterialsView.addControl(MaterialsPanel);		
   }

 if (window.widget){
        menu.setRightSoftkeyLabel("Enter", MaterialsUsed );
        }
 MaterialsLabel.setText(LBL012 + Job + "<br>" + MaterialsButtonText +  " " + totalmaterials[Jobnum]);
 uiManager.setView(MaterialsView);
}
